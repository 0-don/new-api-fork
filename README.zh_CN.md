...
**API 文档：**
- 📖 自动生成 OpenAPI 3.0 规范 — 设置 `ENABLE_OPENAPI=true` 启用，通过 `/openapi.json` 和 `/swagger`（Scalar UI）访问
...
| `ENABLE_OPENAPI` | 在 `/openapi.json` 提供 OpenAPI 规范，在 `/swagger` 提供 Scalar UI | `false` |

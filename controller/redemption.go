...
func AddRedemption(c fuego.ContextWithBody[model.Redemption]) (*dto.Response[[]string], error) {
	...
	if utf8.RuneCountInString(redemption.Name) == 0 || utf8.RuneCountInString(redemption.Name) > 20 {
		return dto.Fail[[]string](common.TranslateMessage(ginCtx, "redemption.name_length"))
	}
	if redemption.Count <= 0 {
		return dto.Fail[[]string](common.TranslateMessage(ginCtx, "redemption.count_positive"))
	}
	if redemption.Count > 100 {
		return dto.Fail[[]string](common.TranslateMessage(ginCtx, "redemption.count_max"))
	}
	...
	for i := 0; i < redemption.Count; i++ {
		...
		if err != nil {
			...
			return &dto.Response[[]string]{
				Message: common.TranslateMessage(ginCtx, "redemption.create_failed"),
				Data:    keys,
			}, nil
		}
...
func validateExpiredTime(c *gin.Context, expired int64) (bool, string) {
	if expired != 0 && expired < common.GetTimestamp() {
		return false, common.TranslateMessage(c, "redemption.expire_time_invalid")
	}
	return true, ""
}

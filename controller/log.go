...
func GetAllLogs(c fuego.ContextWithParams[dto.GetAllLogsParams]) (*dto.Response[dto.PageData[*model.Log]], error) {
	...
	logs, total, err := model.GetAllLogs(p.Type, p.StartTimestamp, p.EndTimestamp, p.ModelName, p.Username, p.TokenName, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), p.Channel, p.Group, p.RequestID, p.UpstreamRequestID, p.SubscriptionPlan)
	if err != nil {
...
func SearchAllLogs(c fuego.ContextNoBody) (dto.MessageResponse, error) {
	return dto.FailMsg(common.TranslateMessage(dto.GinCtx(c), "common.api_deprecated"))
}
...
func GetLogByKey(c fuego.ContextNoBody) (*dto.Response[[]*model.Log], error) {
	...
	if tokenId == 0 {
		return dto.Fail[[]*model.Log](common.TranslateMessage(dto.GinCtx(c), "token.invalid"))
	}
...
func DeleteHistoryLogs(c fuego.ContextWithParams[dto.DeleteHistoryLogsParams]) (*dto.Response[int64], error) {
	...
	if p.TargetTimestamp == 0 {
		return dto.Fail[int64](i18n.Translate("ctrl.target_timestamp_is_required"))
	}
